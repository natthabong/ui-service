angular.module('scfApp').controller(
    'VerifyTransactionController', ['VerifyTransactionService', '$stateParams', 'SCFCommonService', '$scope', '$timeout', '$state', '$filter', 'PageNavigation', 'UIFactory', 'ngDialog', '$log',
        function(VerifyTransactionService, $stateParams, SCFCommonService, $scope, $timeout, $state, $filter, PageNavigation, UIFactory, ngDialog, $log) {
            var vm = this;
            var log = $log;
            vm.isLoanAccount = true;
            vm.dataTable = {
                options: {
                    displayRowNo: {
            ...
            var _loadTradeFinancing = function() {
                var sponsorId = vm.transactionModel.sponsorId;
                var supplierId = vm.transactionModel.supplierId;
                var accountId = vm.transactionModel.payerAccountId;
                var deffered = VerifyTransactionService.getTradeFinanceInfo(sponsorId, supplierId, accountId);
                deffered.promise.then(function(response) {
                    var tradeFinancing = response.data;
                    if (response.data.accountType == 'OVERDRAFT') {
                        vm.isLoanAccount = false;
                    }
                    ...
                    log.error('Cannot load trade financing');
                });
            }
